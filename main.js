...
function deleteCard() {
 var element = event.target.closest('article');
 var elememtID = parseInt(element.dataset.id);
 var index = imagesArray.findIndex(function(photo) {
  return photo.id === elememtID;
 })
 // console.log(index);
...
function favoriteCard() {
  var element = event.target.closest('article');
  var elememtID = parseInt(element.dataset.id);
  var card = imagesArray.find(function(photo) {
    return photo.id === elememtID;
 })
  favoritePhoto();
}
